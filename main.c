  ...
  for (i=0;i<sb->st_size;i++){
    ...
    if(i%gui.ascii_width==0){
      if (i>0) { // we don't want this block to run immediately
        ...
        //mvwprintw(gui.ascii, cursor.y - 1, 0, "%s", tmp_data);
//        wrefresh(gui.ascii);

        // blank the string
        uint8_t w;
        for (w=0; w < sizeof(tmp_data); w++)
          tmp_data[w] = 'x';
        //strcpy(tmp_data, "                     ");
      }
...
//      wprintw(gui.hex, " ");
    ...
    mvwprintw(gui.ascii, cursor.y - 1, i%gui.ascii_width, "%c", tmp_data[i%gui.ascii_width]);
    wrefresh(gui.debug);
  ...
  while (!quit) {
    while (( ch = getch()) != ':') {
      ...
      switch (ch) {
        ...
        case '': // redraw window, recalculating sizes
          setUpGUI();
          test_print(map, &sb);
          break;
      }
